...
namespace AXToolbox.Common
{
    ...
    {
        private static int targetUTMZoneNumber = 0;
        ...
        private CoordAdapter(string sourceDatumName, string targetDatumName)
        {
            try
            {
                datum1 = Datum.GetInstance(sourceDatumName);
            }
            ...
            try
            {
                datum2 = Datum.GetInstance(targetDatumName);
            }
            catch (KeyNotFoundException)
            {
            ...
            performHelmert = sourceDatumName != targetDatumName;
        }

        public static CoordAdapter GetInstance(string sourceDatumName, string targetDatumName)
        {
            CoordAdapter ca;
            var key = sourceDatumName + "/" + targetDatumName;
            ...
            {
                ca = new CoordAdapter(sourceDatumName, targetDatumName);
                adapterCache.Add(key, ca);
            }

            return ca;
        }
        public static void SetTargetUtmZone(string targetUtmZone)
        {
            if (targetUtmZone == "")
            {
                targetUTMZoneNumber = 0;
            }
            else
            {
                targetUTMZoneNumber = int.Parse(targetUtmZone.Substring(0, 2));
            }
        }

        public UtmCoordinates ToUTM(LatLonCoordinates p1)
        {
            ...
            if (performHelmert)
            {
                ...
                p2.OverrideAltitude(p1.Altitude);
#endif
            }
            else
            {
        ...
        public UtmCoordinates ToUTM(UtmCoordinates p1)
        {
            UtmCoordinates p2;

            if (performHelmert || int.Parse(p1.Zone.Substring(0, 2)) != targetUTMZoneNumber)
                return ToUTM(UTMtoLatLong(p1, datum1));
            else
                p2 = p1;
        ...
        private static UtmCoordinates LatLongToUTM(LatLonCoordinates p1, Datum datum)
        {
            //[1]
             ...
             * UTM zone
             */
            int ZoneNumber = targetUTMZoneNumber;
            if (ZoneNumber == 0)
            {
            ...
            var p2 = new UtmCoordinates(
                ZoneNumber.ToString("00") + ComputeUtmLetterDesignator(p1.Latitude.Degrees),

                k0 * N * (A + (1 - T + C) * A * A * A / 6 + (5 - 18 * T + T * T + 72 * C - 58 * ep2) * A * A * A * A * A / 120)
                    + 500000.0,

                k0 * (M + N * Math.Tan(LatRad) * (A * A / 2 + (5 - T + 9 * C + 4 * C * C) * A * A * A * A / 24
