            ...
            switch (ObjectType)
            {
                ...
                case "TDT":
                    ...
                    //TDT(<pointName>, <timeDelay>[, <maxTime>])
                    {
                        ...
                        {
                            if (maxTime.HasValue)
                                Point = new AXWaypoint(ObjectName, Engine.TaskValidTrack.Points.FirstOrDefault(p => p.Time >= referencePoint.Time + timeDelay && p.Time <= maxTime));
                            else
                                Point = new AXWaypoint(ObjectName, Engine.TaskValidTrack.Points.FirstOrDefault(p => p.Time >= referencePoint.Time + timeDelay));
                ...
                case "TDD":
                    ...
                    //TDD(<pointName>, <distanceDelay>[, <maxTime>])
                    {
                        ...
                        {
                            if (maxTime.HasValue)
                                Point = new AXWaypoint(ObjectName, Engine.TaskValidTrack.Points.FirstOrDefault(p => Physics.Distance2D(p, referencePoint) >= distanceDelay && p.Time <= maxTime));
                            else
                                Point = new AXWaypoint(ObjectName, Engine.TaskValidTrack.Points.FirstOrDefault(p => Physics.Distance2D(p, referencePoint) >= distanceDelay));
                ...
                case "TALI":
                    ...
                    //TALI(<areaName>)
                    {
                        ...
                        try
                        {
                            ...
                            Point = new AXWaypoint(ObjectName, tali);
                        }
                        catch
                        {
                            AddNote("no valid track point inside the area", true);
